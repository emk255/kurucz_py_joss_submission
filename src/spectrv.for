...
C     PRDPOW AND PRDDOP ARE PARTIAL REDISTRIBUTION PARAMETERS
    ...
    1 FORMAT(8F10.5)
      ...
      CALL READIN(20)
      CALL DUMPBTABLES(NRHOX,PH1,PC1,PSI1)
...
C     DEBUG: Print interpolation coefficients only every 10th wavelength
      IF(WAVE.GE.300.0D0 .AND. WAVE.LE.310.0D0) THEN
      IF(MOD(NU,10).EQ.0) THEN
 ...
 9996 FORMAT(' DEBUG SPECTRV: NU=',I4,' WAVE=',F10.3,' IEDGE=',I3,
     1 ' C1=',1PE15.8,' C2=',1PE15.8,' C3=',1PE15.8,' SUM=',1PE15.8)
      ...
      ENDIF
      DO 2006 J=1,NRHOX
C     DEBUG: Print opacity only every 10th wavelength and only first/last layer
      IF((J.EQ.1.OR.J.EQ.NRHOX) .AND. WAVE.GE.300.0D0 .AND. 
     1 WAVE.LE.310.0D0 .AND. MOD(NU,10).EQ.0) THEN
      LOGACONT=C1*CONTABS(1,IEDGE,J)+C2*CONTABS(2,IEDGE,J)+
     1 C3*CONTABS(3,IEDGE,J)
      ACONT_COMP=10.**LOGACONT
      WRITE(6,9995)NU,J,IEDGE,LOGACONT,ACONT_COMP
 9995 FORMAT(' DEBUG SPECTRV: NU=',I4,' J=',I3,' IEDGE=',I3,
     1 ' LOGACONT=',1PE15.8,' ACONT=',1PE15.8)
      ENDIF
...
C      IF(IFOP(17).EQ.1)CALL XLINOP
   ...
   33 FORMAT(1H1)
      READ(9)ASYNTH
C     DEBUG: Print ASYNTH and ALINE for problematic wavelengths
      DEBUG_WAVE=.FALSE.
      IF(WAVE.GE.300.0D0 .AND. WAVE.LE.310.0D0) THEN
         IF(DABS(WAVE-300.911D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-300.901D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-300.189D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-300.179D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-302.681D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-302.671D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-302.500D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-302.490D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-306.009D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DABS(WAVE-305.999D0).LT.0.01D0) DEBUG_WAVE=.TRUE.
         IF(DEBUG_WAVE) THEN
            WRITE(6,9986)NU,WAVE
 9986       FORMAT(' DEBUG SPECTRV LINE: NU=',I4,' WAVE=',F10.3)
         ENDIF
      ENDIF
      DO 241 J=1,NRHOX
C      ALINE(J)=AHLINE(J)+ASYNTH(J)*(1.-FSCAT(J))+AXLINE(J)
      ALINE(J)=ASYNTH(J)*(1.-FSCAT(J))
C     DEBUG: Print ASYNTH, FSCAT, ALINE for problematic wavelengths
      IF(DEBUG_WAVE) THEN
         IF(J.EQ.1 .OR. J.EQ.NRHOX) THEN
            WRITE(6,9985)NU,J,WAVE,ASYNTH(J),FSCAT(J),ALINE(J),SLINE(J),
     1 ACONT(J)
 9985       FORMAT(' DEBUG SPECTRV LINE: NU=',I4,' J=',I3,' WAVE=',F10.3,
     2 ' ASYNTH=',1PE15.8,' FSCAT=',1PE15.8,' ALINE=',1PE15.8,
     3 ' SLINE=',1PE15.8,' ACONT=',1PE15.8)
         ENDIF
      ENDIF
C      SLINE(J)=BNU(J)
C     ASSUMES BUP=1
...
c     29oct07 replaced missing comma after OTHER2  
  ...
  111 CALL EXIT
      END
      SUBROUTINE DUMPBTABLES(NDEPTH,PH1,PC1,PSI1)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER (kw=99)
      INTEGER NDEPTH
      REAL*8 PH1,PC1,PSI1
     ...
     2             SHLINE(kw),XNFPH(kw,2),XNFH(kw)
      COMMON /BSI/BSI1(kw,11),ASI1(kw),SSI1(kw),XNFPSI(kw,2),BSI2(kw,10)
      INTEGER J
      OPEN(UNIT=90,FILE='btables.dat',STATUS='UNKNOWN')
      WRITE(90,'(A,I5)') '# NRHOX ',NDEPTH
      WRITE(90,'(A,3(1X,1P,E22.14))') '# PH1_PC1_PSI1 ',
     1 PH1,PC1,PSI1
      WRITE(90,'(A)') '# BHYD BMIN'
      DO 1010 J=1,NDEPTH
 1010 WRITE(90,2010) J,(BHYD(J,I),I=1,8),BMIN(J)
      WRITE(90,'(A)') '# BC1'
      DO 1020 J=1,NDEPTH
 1020 WRITE(90,2020) J,(BC1(J,I),I=1,14)
      WRITE(90,'(A)') '# BC2'
      DO 1030 J=1,NDEPTH
 1030 WRITE(90,2030) J,(BC2(J,I),I=1,6)
      WRITE(90,'(A)') '# BSI1'
      DO 1040 J=1,NDEPTH
 1040 WRITE(90,2040) J,(BSI1(J,I),I=1,11)
      WRITE(90,'(A)') '# BSI2'
      DO 1050 J=1,NDEPTH
 1050 WRITE(90,2050) J,(BSI2(J,I),I=1,10)
      CLOSE(UNIT=90)
 2010 FORMAT(I5,1X,1P9E20.12)
 2020 FORMAT(I5,1X,1P14E20.12)
 2030 FORMAT(I5,1X,1P6E20.12)
 2040 FORMAT(I5,1X,1P11E20.12)
 2050 FORMAT(I5,1X,1P10E20.12)
      RETURN
      END
      SUBROUTINE LINCEN
      RETURN
