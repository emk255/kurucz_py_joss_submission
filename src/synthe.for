...
C      PARAMETER (LENREC=8000,MAXLEN=2000001,MAXPROF=10000,
...
C     DEBUG: Print TK values read from fort10
      WRITE(6,*)'DEBUG SYNTHE: TK values read from fort10'
      DO 1891 J=1,MIN(5,NRHOX)
      TK_EXPECTED=1.38054E-16*QT(J)
      WRITE(6,1892)J,QT(J),QTK(J),TK_EXPECTED
 1892 FORMAT(' DEBUG SYNTHE READ: J=',I3,' T=',1PE12.4,' TK_read=',E12.4,
     1 ' TK_expected=',E12.4)
 1891 CONTINUE
      DO 2010 J=1,NRHOX
      T(J)=QT(J)
      ...
      P(J)=QP(J)
      XNE(J)=QXNE(J)
C     DEBUG: Log XNE values read from fort.10
      IF(J.LE.5)WRITE(6,1893)J,XNE(J)
 1893 FORMAT(' DEBUG SYNTHE: XNE read from fort.10 J=',I3,' XNE=',E12.4)
      XNATOM(J)=QXNATOM(J)
      RHO(J)=QRHO(J)
      RHOX(J)=QRHOX(J)
      VTURB(J)=QVTURB(J)
...
C     SAVE PRE-BOLTZMANN KAPPA0 FOR DEBUG
      KAPPA0PRE=KAPPA0
      KAPPA0=KAPPA0*FASTEX(ELO*HCKT(J))
C     KAPPA0=KAPPA0*EXP(-ELO*HCKT(J))
      IF(KAPPA0.LT.KAPMIN)GO TO 350
C     DEBUG: Print fort.12 line opacity values for 300.9 nm
C     WAVEL=WBEGIN*RATIO**(NBUFF-1) gives wavelength for this buffer index
      WAVEL12=WBEGIN*RATIO**(NBUFF-1)
      IF(WAVEL12.GE.300.89D0.AND.WAVEL12.LE.300.92D0.AND.J.EQ.1)THEN
      BOLT12=FASTEX(ELO*HCKT(J))
      WRITE(6,8001)ILINE,WAVEL12,NELION,CONGF,QXNFDOP(NELION),
     1 ELO,HCKT(J),BOLT12,KAPPA0PRE,KAPPA0
 8001 FORMAT(' DEBUG FORT12: ILINE=',I7,' WL=',F10.4,' NELION=',I4,
     1 ' CONGF=',1PE12.4,' XNFDOP=',1PE12.4,/,
     2 '   ELO=',1PE12.4,' HCKT=',1PE12.4,' BOLT=',1PE12.4,
     3 ' KAP0_PRE=',1PE12.4,' KAP0=',1PE12.4)
      ENDIF
C
...
C
     ...
     1EXTABF(IFIX((X-FLOAT(IFIX(X)))*1000.+1.5))
C
      IF(.NOT.XLDBGOPEN)THEN
      OPEN(UNIT=199,FILE='xlinop_debug.log',STATUS='UNKNOWN')
      WRITE(199,9053) 'ENTER XLINOP'
      XLDBGOPEN=.TRUE.
      ENDIF
...
c
      IF(ILINE.LE.5) WRITE(199,9052) ILINE,WL
      IF(WL.GE.280.D0 .AND. WL.LE.340.D0) THEN
      IF(.NOT.XLDBGOPEN)THEN
      OPEN(UNIT=199,FILE='xlinop_debug.log',STATUS='UNKNOWN')
      WRITE(199,9053) 'ENTER XLINOP'
      XLDBGOPEN=.TRUE.
      ENDIF
      WRITE(199,9054) ILINE,WL,ELO,GF,NELION,GAMMAR,GAMMAS,GAMMAW
      ENDIF
      WL=WL*DOPRATIO
...
C     DEBUG: dump damping and opacity inputs for metal lines near 300 nm
      IF(.NOT.XLDBGOPEN)THEN
      OPEN(UNIT=199,FILE='xlinop_debug.log',STATUS='UNKNOWN')
      XLDBGOPEN=.TRUE.
      ENDIF
C     trace first line to verify WL scale
      IF(ILINE.EQ.1) WRITE(199,9050) WL
      IF(ILINE.LE.50) WRITE(199,9052) ILINE,WL
 9050 FORMAT(' DEBUG XLINOP: FIRST LINE WL=',1PE12.4)
 9052 FORMAT(' DEBUG XLINOP: TRACE ILINE=',I7,' WL=',1PE12.4)
 9053 FORMAT(A)
 9054 FORMAT(' DEBUG XLINOP: INPUT ILINE=',I7,' WL=',1PE12.4,' ELO=',1PE12.4,
     1 ' GF=',1PE12.4,' NELION=',I4,' GR=',1PE12.4,' GS=',1PE12.4,
     2 ' GW=',1PE12.4)
      IF(WL.GE.280.D0 .AND. WL.LE.340.D0) THEN
      WRITE(199,9051) ILINE,WL,ELO,GF,NELION,KAPPA0,ADAMP,
     1 GAMMAR,GAMMAS,GAMMAW,TXNXN(J),XNE(J),DOPPLE(NELION),
     2 XNFDOP(NELION)
 9051 FORMAT(' DEBUG XLINOP: ILINE=',I7,' WL=',F10.4,' ELO=',1PE12.4,
     1 ' GF=',1PE12.4,' NELION=',I4,' KAP0=',1PE12.4,' ADAMP=',
     2 1PE12.4,' GR=',1PE12.4,' GS=',1PE12.4,' GW=',1PE12.4,
     3 ' TXNXN=',1PE12.4,' XNE=',1PE12.4,' DOP=',1PE12.4,
     4 ' XNFDOP=',1PE12.4)
      ENDIF
      KAPCEN=KAPPA0*VOIGT(0.,ADAMP)
      IF(LINOUT.GE.0)WRITE(15)ILINE,KAPCEN
